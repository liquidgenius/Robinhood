...
class Robinhood:
    ...
    endpoints = {
        ...
        "sp500_dn": "https://api.robinhood.com/midlands/movers/sp500/?direction=dn",
    ...
    def get_sp500_up(self):
        """fetch S&P 500 up endpoint
        Args:
            stock (str): stock ticker

        Returns:
            (:obj:`dict`) values returned from `sp500_up` endpoint

        """
        res = self.session.get(self.endpoints['sp500_up'])
        res = res.json()
        return res['results']

    def get_sp500_dn(self):
        """fetch S&P 500 down endpoint
        Args:
            stock (str): stock ticker

        Returns:
            (:obj:`dict`) values returned from `sp500_dn` endpoint

        """
        res = self.session.get(self.endpoints['sp500_dn'])
        res = res.json()
        return res['results']
    ...
    def place_order(
        ...
        if not bid_price and order == Order.MARKET:
            bid_price = self.quote_data(instrument['symbol'])['results'][0]['bid_price']
    ...
    def place_daily_stop_loss_sell_order(
            self,
            instrument,
            quantity,
            bid_price=None,
        ...
        """wrapper for placing stop limit orders

        Args:
            instrument (dict): the RH URL and symbol in dict for the instrument to be traded
            quantity (int): quantity of stocks in order
            stop_price (float): the price at which the stop will trigger (?)
            (see coding around line #860-61 regarding bid_price)

        Returns:
            (:obj:`requests.request`): result from `orders` put command

        Usage:
            >> rh.login(username=config.rh['user'], password=config.rh['pass'])
            >> full_inst = dict(rh.instruments(symbol='AAPL'))
            >> inst = {'url' : full_inst.get('url'), 'symbol' : full_inst.get('symbol')}
            >> rh.place_daily_stop_loss_sell_order(instrument=inst, quantity=1, bid_price=None, stop_price=114.0)

        """
        transaction = Transaction.SELL
        return self.place_order(instrument, quantity, transaction, bid_price, stop_price, trigger, order, time_in_force)

    def place_temporary_stop_loss_sell_order(
            # TODO Needs testing
            self,
            instrument,
            quantity,
            ...
            trigger = Trigger.STOP,
            order = Order.MARKET,
            time_in_force = TimeForce.GTC,
    ):

        """wrapper for placing temporary stop limit orders, replace often
        ...
        Usage:
            ...
            >> full_inst = dict(rh.instruments(symbol='AAPL'))
            >> inst = {'url' : full_inst.get('url'), 'symbol' : full_inst.get('symbol')}
            >> rh.place_temporary_stop_loss_sell_order(instrument=inst, quantity=1, bid_price=None, stop_price=114.0)
